...
import { beginRegistration, getAuthConfig, type TOTPEnrollment } from '../services/http'
...
const addressPreview = computed(() => {
  const local = displayName.value.trim().toLocaleLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/^-|-$/g, '')
...
async function begin() {
  error.value = ''
  try {
    const result = await beginRegistration(displayName.value, recoveryEmail.value, challenge.value)
  ...
  <main class="page shell auth-page">
    <section class="auth-panel auth-panel-wide">
      ...
      <form class="auth-form" @submit.prevent="begin">
        ...
        <input id="register-name" v-model="displayName" autocomplete="name" required autofocus />
