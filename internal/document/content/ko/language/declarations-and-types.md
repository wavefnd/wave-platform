...
| --- | --- |
| `var` | 재대입 가능한 지역 변수 |
| `let` | 재대입할 수 없는 지역 바인딩 |
| `let mut` | 재대입 가능한 `let` 바인딩 |

`let`에 다시 값을 대입하면 컴파일 단계에서 거부됩니다.
...
v0.2.0-pre-beta에서 `const`와 `static`은 최상위에서 사용합니다. 함수 블록 안의 지역 선언은 `var` 또는 `let` 계열을 사용하십시오.
...
`isz`와 `usz`는 lexer가 인식하지만 이 릴리스의 타입 변환 경로에서 처리되지 않습니다. v0.2.0-pre-beta 문서와 예제에서는 사용하지 마십시오.
